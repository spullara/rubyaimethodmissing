...
puts "Result: #{result1}, API calls so far: #{$api_calls}"
puts ""

puts "Test 2: Call fibonacci(10) again - should use cache (no API call)"
result2 = calc.fibonacci(10)
puts "Result: #{result2}, API calls so far: #{$api_calls}"
puts ""

puts "Test 3: Call factorial(5) - should trigger API call"
result3 = calc.factorial(5)
puts "Result: #{result3}, API calls so far: #{$api_calls}"
puts ""

puts "Test 4: Call factorial(5) again - should use cache (no API call)"
result4 = calc.factorial(5)
puts "Result: #{result4}, API calls so far: #{$api_calls}"
puts ""

puts "=" * 60
if $api_calls == 2
  puts "✓ CACHE WORKING: Only 2 API calls for 4 method invocations"
else
  puts "✗ CACHE ISSUE: Expected 2 API calls, got #{$api_calls}"
end
puts "=" * 60
